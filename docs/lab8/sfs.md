...
文件系统通常保存在磁盘上。但在本实验中，我们采用内存模拟磁盘。我们在ucore编译时会生成一个initrd镜像（即ucore中的disk0）用于存放一个SFS文件系统（Simple
Filesystem），它是一个使用内存模拟的磁盘，并将磁盘镜像与内核链接在一起与内核一起加载到内存。通常文件系统中，磁盘的使用是以扇区（Sector）为单位的，但是为了实现简便，SFS
